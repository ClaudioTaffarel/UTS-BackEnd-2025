...
async function getCharacterById(request, response, next) {
  try {
    const character = await charactersService.getCharacterById(
      request.params.id,
    );
...
async function createCharacters(request, response, next) {
  try {
    const { name, description, gender, race } = request.body;
    ...
    return response
      .status(200)
      .json({ message: "Character Data created successfully" });
  } catch (error) {
    return next(error);
  }
}
